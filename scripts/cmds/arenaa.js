...
${CONFIG.DEFAULT_PETS.map(p => `${p.icon} ${p.name}`).join(', ')}
    ...
    if (userPets.length === 0) {
      userPets = CONFIG.DEFAULT_PETS.map(p => ({ ...p, level: 1 }));
      await usersData.set(senderID, { ...userData, pets: userPets });
    }

    const myPetData = userPets.find(p => p.name.toLowerCase() === petName.toLowerCase());
    if (!myPetData) {
      const msg = `❌ Pet "${petName}" non trouvé. Utilisez !mypets pour voir vos pets.`;
    ...
    if (isAIMode) {
      ...
      const replyMsg = await message.reply(`⚔️ ${message.author} défie les autres joueurs ! Envoyez le nom de votre pet pour combattre.`);
...
async function playTurn(message, gameId) {
  ...
  if (isAI) {
    ...
    const prompt = `
${active.getPlayerUI({ showStats: true, selectionOptions: true })}
...
Choisissez votre action.
    ...
    const collected = await new Promise(resolve => {
      global.GoatBot.onReply.set(replyMsg.messageID, {
        callback: (ctx) => resolve(ctx),
        author: game.activePlayer === 1 ? game.player1Author : game.player2Author,
        filter: (ctx) => true
      });
    });
    if (!collected) {
    ...
    move = collected.body.trim().toLowerCase();
    if (!CONFIG.MOVES[move]) {
      await message.reply("❌ Mouvement invalide. Réessayez.");
      return playTurn(message, gameId);
    }
  }
