...
fr: "Ajouter, retirer le rôle premium avec durée"
},
category: "owner",
guide: {
vi: ' {pn} [add | -a] <uid | @tag> [time]: Thêm quyền premium cho người dùng',
en: ' {pn} [add | -a] <uid | @tag> [time]: Add premium role for user',
fr: ' {pn} add -a <uid | @tag> [temps]: Ajouter le premium à un user'
        + '\n Temps: jours (1d), heures (2h), minutes (30m) ou permanent'
        + '\n Ex: {pn} add @user 7d (7 jours)'
        + '\n {pn} remove -r <uid | @tag>: Retirer le premium'
        + '\n {pn} list -l: Voir la liste des premium'
        + '\n {pn} check -c <uid | @tag>: Vérifier le temps restant'
}
},
...
langs: {
            ...
            fr: {
                    added: "✅ | Rôle premium ajouté à %1 user(s):\n%2",
                    alreadyPremium: "\n⚠️ | %1 user(s) étaient déjà premium:\n%2",
                    missingIdAdd: "⚠️ | Entre un ID ou tag le user à mettre premium",
                    removed: "✅ | Rôle premium retiré à %1 user(s):\n%2",
                    notPremium: "⚠️ | %1 user(s) n'étaient pas premium:\n%2",
                    missingIdRemove: "⚠️ | Entre un ID ou tag le user à retirer",
                    listPremium: "★ | Liste des utilisateurs Premium:\n%1",
                    premiumInfo: "✅ | Info Premium de %1:\n• Statut: %2\n• Expire: %3",
                    invalidTime: "⚠️ | Format temps invalide! Utilise: 1d (jours), 2h (heures), 30m (minutes) ou permanent",
                    permanent: "Permanent",
                    expires: "Il reste %1",
                    expired: "Expiré"
            }
    ...
    onStart: async function ({ message, args, usersData, event, getLang }) {
            ...
            const getTimeRemaining = (expireTime) => {
                    ...
                    if (days > 0) return getLang("expires", `${days}j ${hours}h`);
            ...
            switch (args[0]) {
                    ...
                    case "-c": {
                            ...
                            const expireTime = await usersData.get(uid, "data.premiumExpireTime");
                            const status = isPremium? "Premium" : "Pas Premium";
